...
internal class SimplyJson(private val context: Context, private val payload: String) {
    ...
    private val pageConverters: HashMap<String, PageConverter> by lazy {
        HashMap<String, PageConverter>().also { map ->
    ...
    private val composerConverters: HashMap<String, ComposerConverter> by lazy {
        HashMap<String, ComposerConverter>().also { map ->
    ...
    suspend fun generateWith(
        ...
        for (index in 0 until contentArray.length()) {
            if ((contentArray.getJSONObject(index).has(Node.TYPE)) &&
                ...
                generateWith(
                    SetupHandler(pdfBuilder, contentArray.getJSONObject(index).toString())
